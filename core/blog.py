...
# Additional manually curated depth for the 6.1.1 AdSense quality pass.
# These are editorial sections, not generated from tool metadata at request time.
EXTRA_SECTIONS = {
    "word-to-pdf-without-losing-formatting": [
        ("اختبر الروابط والعناصر التي ينقر عليها القارئ", "إذا كان المستند يحتوي على روابط لمراجع أو مواقع أو عناوين بريد، لا تكتفِ بأن شكلها بقي أزرق بعد التحويل. افتح PDF الناتج واضغط على رابط أو اثنين، لأن المظهر وحده لا يثبت أن العنصر بقي قابلًا للنقر. هذا مهم خصوصًا للسيرة الذاتية والتقارير التي تعتمد على مراجع خارجية.", "Test links and other clickable elements", "If the document contains links to references, websites, or email addresses, do not assume they survived just because they still look blue. Open the finished PDF and click a couple of them. This matters especially for CVs and reports where an external reference is part of the document's usefulness."),
    ],
    "pdf-to-word-when-it-works-best": [
        ("التخطيط متعدد الأعمدة يحتاج توقعات واقعية", "الملفات التي تشبه المجلات أو النشرات قد تحتوي على أعمدة وصناديق نص وعناصر موضوعة بدقة. عند تحويلها إلى Word قد يحافظ البرنامج على الكلمات لكنه يعيد بناء التخطيط بطريقة مختلفة. إذا كان هدفك التعديل، ركّز أولًا على سلامة النص ثم رتّب التصميم النهائي بدل مطاردة تطابق بصري كامل من أول محاولة.", "Multi-column layouts need realistic expectations", "Magazine-like PDFs can contain columns, text boxes, and precisely positioned objects. A Word conversion may recover the words while rebuilding the layout differently. If editing is the goal, verify the text first and then refine the final layout instead of expecting pixel-perfect reconstruction from the first pass."),
    ],
    "pdf-compression-without-making-it-ugly": [
        ("تجنب سلسلة ضغط فوق ضغط", "إذا ضغطت ملفًا ثم أخذت النسخة المضغوطة وضغطتها مرة ثانية وثالثة، قد تتراكم الخسارة خصوصًا في الصور. احتفظ بالأصل وابدأ منه عندما تحتاج نسخة بحجم مختلف. بهذه الطريقة تقارن إعدادات متعددة من مصدر واحد بدل أن تبني كل نسخة على نسخة أضعف منها.", "Avoid a chain of repeated compression", "Compressing an already compressed copy again and again can accumulate visible loss, especially in images. Keep the original and create each size variant from that source. You can then compare settings fairly instead of building every new copy from a weaker previous copy."),
    ],
    "ocr-for-scanned-arabic-pdfs": [
        ("احتفظ بصورة الصفحة كمرجع للتدقيق", "حتى عندما يكون النص المستخرج ممتازًا، من المفيد إبقاء الصفحة الأصلية بجانب النص أثناء المراجعة. هذا يجعل التحقق من الأسماء والأرقام وعلامات الترقيم أسرع، ويمنعك من التعامل مع نتيجة OCR كأنها النسخة القانونية أو الأكاديمية الأصلية للمستند.", "Keep the page image as a proofreading reference", "Even when extracted text looks excellent, keep the original page beside it while proofreading. That makes names, numbers, and punctuation easier to verify and prevents the OCR output from being mistaken for the legal or academic source document."),
    ],
    "jpg-png-webp-which-image-format": [
        ("انتبه للبيانات الوصفية والألوان", "تغيير الصيغة قد يغيّر أو يزيل بعض البيانات الوصفية، وقد تختلف الألوان قليلًا بين التطبيقات إذا كانت الصورة تعتمد على ملف لوني خاص. للصور اليومية هذا نادرًا ما يكون مشكلة، لكن أعمال الطباعة أو الصور التي تحتاج معلومات EXIF تستحق اختبار نسخة قبل تحويل مكتبة كاملة.", "Remember metadata and color information", "Changing format can remove some metadata, and colors may look slightly different between applications when an image depends on a specific color profile. That is rarely a problem for everyday sharing, but print work or photos that need EXIF data deserve a test conversion before processing a whole library."),
    ],
    "reduce-image-size-for-web": [
        ("اختبر الصورة في الصفحة الحقيقية", "أفضل مقارنة ليست داخل أداة الضغط فقط. ضع النسخة المصغرة في المكان الذي ستظهر فيه فعلًا وشاهدها على جوال وشاشة أكبر. قد تكتشف أن ملفًا أصغر بكثير يبدو مطابقًا تقريبًا عند حجم العرض الحقيقي، أو أن نصًا صغيرًا يحتاج جودة أعلى من بقية الصور.", "Test the image in its real page", "The best comparison is not inside the compression tool alone. Put the smaller file where it will actually appear and view it on a phone and a larger screen. You may find that a much smaller file looks nearly identical at real display size, or that small embedded text needs more quality than ordinary photos."),
    ],
    "merge-pdfs-cleanly": [
        ("راجع الترقيم والإشارات الداخلية بعد الدمج", "إذا كانت الملفات الأصلية تحتوي أرقام صفحات مطبوعة أو إشارات مثل انظر الصفحة 12، قد يصبح الترتيب الجديد مختلفًا بعد الدمج. راجع هذه الإشارات في المستندات الرسمية الطويلة. الدمج التقني قد يكون صحيحًا تمامًا بينما يحتاج المحتوى نفسه إلى تحديث رقم أو مرجع.", "Recheck page numbering and internal references", "If source files contain printed page numbers or references such as 'see page 12,' the new merged order can make those references inaccurate. Review them in long formal documents. The merge can be technically perfect while the document itself still needs a page number or reference updated."),
    ],
    "zip-files-without-the-mess": [
        ("تذكر أن التوافق أهم من نسبة ضغط صغيرة", "بعض إعدادات الأرشفة المتقدمة توفر حجمًا أقل لكنها تحتاج برنامجًا معينًا عند الطرف الآخر. إذا كان الملف سيذهب إلى عميل أو جامعة أو جهة لا تعرف جهازها، اختبر الأرشيف ببرنامج شائع أو استخدم ZIP عاديًا. توفير بضعة ميجابايت لا يفيد إذا لم يستطع المستلم فتح الملف.", "Compatibility can matter more than a small size gain", "Some advanced archive settings save a little more space but require specific software at the other end. If the archive is going to a client, university, or someone whose device you do not control, test it with common software or use a conventional ZIP. Saving a few megabytes is not useful if the recipient cannot open the file."),
    ],
    "privacy-first-file-conversion": [
        ("عامل حساسية الملف كقرار مستقل", "ليست كل الملفات متساوية. صورة عامة وملف يحتوي هوية أو بيانات مالية لا ينبغي أن يُعاملا بنفس الطريقة. قبل الرفع اسأل هل هذه المهمة تستحق استخدام خدمة عبر الإنترنت أصلًا، وهل يمكن حذف صفحات أو حقول لا تحتاجها. الخصوصية الجيدة تبدأ بتقليل البيانات قبل أن تبدأ الأداة عملها.", "Treat file sensitivity as a separate decision", "Not every file deserves the same handling. A public image and a document containing identity or financial data should not be treated alike. Before uploading, ask whether the job needs an online service at all and whether unnecessary pages or fields can be removed. Good privacy starts by minimizing data before a tool processes it."),
    ],
    "doc-vs-docx-what-to-use": [
        ("لا تستخدم الحفظ باسم كاختبار وحيد للتوافق", "قد يفتح البرنامج ملف DOC قديمًا ثم يحفظه كـDOCX بدون مشكلة ظاهرة، لكن عناصر مثل الماكرو أو الحقول أو التخطيطات القديمة قد تتصرف بشكل مختلف. إذا كان الملف مهمًا، قارن صفحتين أو ثلاثًا بعد الحفظ ولا تحذف النسخة القديمة فورًا.", "Do not treat Save As as the only compatibility test", "An application may open an old DOC and save it as DOCX without an obvious warning, while legacy fields, macros, or layout features can still behave differently. For an important document, compare several pages after saving and keep the old copy until you are satisfied."),
        ("اختر الصيغة حسب الشخص الذي سيكمل العمل", "إذا كنت سترسل الملف لشخص سيعدله، اسأله عن البرنامج أو النظام الذي يستخدمه بدل افتراض أن كل شيء متطابق. أما إذا كانت النسخة نهائية للقراءة فقط، فغالبًا PDF يقلل اختلافات العرض. الصيغة الأفضل هي التي تناسب الخطوة التالية في سير العمل.", "Choose the format for the person doing the next step", "If someone else will edit the file, ask what software or system they use rather than assuming every office suite behaves identically. If the document is final and only needs to be read, PDF often reduces display differences. The best format is the one that fits the next step in the workflow."),
    ],
    "images-to-pdf-clean-document": [
        ("الإضاءة والمنظور يسبقان التحويل", "إذا كانت الصور ملتقطة بالجوال، حاول أن تكون الكاميرا موازية للورقة وأن تكون الإضاءة موزعة بدون ظل قوي على أحد الأطراف. تحويل صورة مائلة أو مظلمة إلى PDF لا يصلح أصل الصورة تلقائيًا؛ هو فقط يضعها داخل مستند. صورة جيدة من البداية تعني PDF أوضح وأخف وأسهل للقراءة.", "Lighting and perspective come before conversion", "For phone photos, keep the camera as parallel to the page as possible and use even light without a strong shadow across one edge. Turning a dark or skewed photo into PDF does not repair the source; it only places that image into a document. A clean source produces a clearer, often smaller, and easier-to-read PDF."),
        ("وحّد الهوامش قدر الإمكان", "إذا كانت صفحة مقصوصة بإحكام جدًا وأخرى حولها فراغ كبير، سيبدو الملف النهائي غير متزن حتى لو كانت كل الصفحات صحيحة. حاول استخدام قص متقارب واتجاه واحد للصور المتشابهة، خصوصًا للواجبات والنماذج متعددة الصفحات.", "Keep margins reasonably consistent", "If one page is tightly cropped while another has a large empty border, the final document can feel uneven even when every page is readable. Use similar cropping and orientation for comparable pages, especially for assignments and multi-page forms."),
    ],
    "password-protected-pdf-before-sharing": [
        ("الحماية بكلمة مرور ليست حجبًا للمعلومات", "إذا كان داخل الملف رقم أو سطر يجب ألا يراه المستلم، لا تعتمد على كلمة المرور وحدها؛ بمجرد فتح الملف يستطيع الشخص رؤية المحتوى المسموح له بفتحه. في هذه الحالة استخدم حجبًا أو إزالة حقيقية للمعلومة في نسخة المشاركة، ثم راجع النتيجة بصريًا.", "Password protection is not redaction", "If the file contains a number or line the recipient must never see, a password alone is not enough. Once the file is opened, its visible contents are available to the authorized reader. Use real redaction or removal in the sharing copy, then inspect the result visually."),
        ("أرسل كلمة المرور بقناة منفصلة عندما يهم الأمر", "للملفات الحساسة، إرسال الملف وكلمة المرور في الرسالة نفسها يقلل فائدة الحاجز. يمكنك مشاركة الملف في قناة وإرسال كلمة المرور في قناة أخرى موثوقة. ليست هذه حماية مطلقة، لكنها ممارسة أبسط وأفضل من وضع المفتاح بجانب القفل.", "Use a separate channel for the password when it matters", "For sensitive files, sending the document and password in the same message weakens the extra barrier. Share the file through one trusted channel and the password through another when practical. It is not absolute security, but it is better than placing the key beside the lock."),
    ],
    "word-to-pdf-on-mobile": [
        ("تأكد من أنك تختار النسخة الأحدث", "تطبيقات الملفات والسحابة قد تعرض نسخًا متشابهة بأسماء قريبة. قبل التحويل انظر إلى تاريخ التعديل أو افتح آخر صفحة فيها تعديل واضح. الخطأ الأكثر إحباطًا ليس فشل التحويل، بل تحويل نسخة قديمة ثم رفعها وأنت تظن أنها الجديدة.", "Make sure you select the latest copy", "Cloud and file apps can show several similarly named versions. Before converting, check the modification time or open a page containing a recent edit. One of the most frustrating mistakes is not a failed conversion; it is successfully converting and submitting an older copy."),
        ("لا تعدّل PDF النهائي إذا كان المصدر ما زال متاحًا", "إذا اكتشفت خطأ في كلمة أو رقم بعد التحويل، الأفضل غالبًا تعديل ملف Word الأصلي ثم إنشاء PDF جديد. التعديل المتكرر على النسخة النهائية قد يصنع اختلافًا بين المصدر والنسخة المرسلة ويصعب معرفة أيهما الأحدث.", "Edit the source when possible, not the final PDF", "If you spot a word or number that needs changing after conversion, it is usually cleaner to edit the original Word document and create a new PDF. Repeatedly patching the final copy can leave the editable source and the submitted version out of sync."),
    ],
    "best-image-format-for-print": [
        ("احسب الدقة بالنسبة للحجم المطبوع", "عدد البكسلات وحده لا يكفي بدون معرفة حجم الطباعة. صورة بعرض 3000 بكسل قد تكون ممتازة في مقاس وتصبح محدودة في ملصق كبير. إذا أعطتك المطبعة قيمة PPI أو DPI مستهدفة، استخدمها مع الأبعاد الفعلية بدل تحويل الامتداد ومحاولة تعويض نقص الدقة.", "Relate pixel dimensions to the printed size", "Pixel count only makes sense together with the intended physical size. A 3000-pixel image may be excellent at one size and limited on a large poster. If the printer gives you a target PPI or DPI, use it with the real dimensions instead of changing the file extension and hoping it will make up for missing resolution."),
        ("اسأل عن ملف الألوان في الأعمال الحساسة", "للطباعة المنزلية والعادية قد لا تحتاج أكثر من معاينة جيدة، لكن الأعمال التجارية الحساسة للألوان قد تتطلب ملفًا لونيًا أو إعدادات من المطبعة. لا تحول كل شيء عشوائيًا إلى صيغة مختلفة قبل أن تعرف متطلبات الجهة التي ستطبع العمل.", "Ask about color requirements for color-critical work", "Ordinary home printing may need little more than a careful preview, but color-critical commercial work can require a specific profile or printer guidance. Do not convert everything to a different format at random before learning what the print provider expects."),
    ],
    "scanned-pdf-ocr-workflow": [
        ("صحح الميل قبل معالجة مئات الصفحات", "إذا لاحظت أن الصفحات تميل بالزاوية نفسها، عالج المشكلة من المصدر أو صحح الاتجاه قبل تشغيل OCR على الملف كاملًا. خطأ صغير يتكرر على 200 صفحة يتحول إلى مئات الفرص لأخطاء التعرف، لذلك اختبار خمس صفحات أولًا يوفر وقتًا كبيرًا.", "Fix repeated skew before processing hundreds of pages", "If pages share the same visible tilt, correct the source or orientation before running OCR over the whole document. A small defect repeated across 200 pages creates hundreds of extra recognition opportunities for error, so a five-page pilot can save a great deal of cleanup."),
        ("اختبر طبقة البحث وليس شكل الصفحة فقط", "في PDF القابل للبحث قد تبقى صورة الصفحة كما هي بينما تُضاف طبقة نص خلفها. بعد OCR جرّب البحث عن اسم ورقم ونسخ سطر إلى محرر نص. إذا ظهرت الصفحة جميلة لكن البحث يفشل أو النص المنسوخ مشوه، فما زالت طبقة OCR تحتاج مراجعة.", "Test the searchable layer, not only the page image", "A searchable PDF can keep the original page image while adding an invisible text layer. After OCR, search for a name and a number and copy a sentence into a text editor. If the page looks perfect but search fails or copied text is garbled, the OCR layer still needs attention."),
    ],
}

for _post in BLOG_POSTS:
    for _heading_ar, _paragraph_ar, _heading_en, _paragraph_en in EXTRA_SECTIONS.get(_post["slug"], []):
        _post["sections"].append((_heading_ar, _paragraph_ar))
        _post["section_en"].append(_heading_en)
        _post["paragraph_en"].append(_paragraph_en)
