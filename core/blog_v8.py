...
_AR_REVIEW_TAIL = {
    "repair-pdf-before-converting": "ولو كان الملف مهمًا للعمل أو الدراسة، دوّن سبب الإصلاح والنسخة التي اعتمدتها حتى لا تختلط عليك الملفات لاحقًا عند المقارنة أو المشاركة.",
    "searchable-pdf-vs-ocr-text": "ومن المفيد الاحتفاظ بصفحة اختبار مرجعية تقارن بها أي إعداد جديد، خصوصًا إذا كان المستند عربيًا أو يحتوي جداول وأرقامًا دقيقة.",
    "remove-image-metadata-before-sharing": "وأخيرًا افتح نسخة المشاركة نفسها بعد الحفظ وتأكد أن الصورة المطلوبة ما زالت واضحة وأنك لم تحذف الأصل الذي قد تحتاج بياناته لاحقًا.",
    "xlsx-to-csv-without-surprises": "وإذا كانت البيانات ستدخل نظامًا مهمًا، احتفظ بعينة تحقق معروفة النتائج حتى تكتشف أي تغيير في الترميز أو الأعمدة قبل استيراد الملف الكامل.",
    "csv-to-json-for-apis": "ومن الأفضل حفظ مثال JSON صحيح متفق عليه مع النظام المستهدف، ثم مقارنة الناتج الجديد به قبل أي إرسال جماعي أو تحديث لبيانات حقيقية.",
    "favicon-pack-from-one-image": "واحتفظ بصورة المصدر عالية الجودة منفصلة عن الحزمة النهائية، لأنك قد تحتاج إعادة التوليد لاحقًا لمقاسات أو متطلبات منصة مختلفة.",
    "reorder-and-number-pdf-pages": "ولو كان المستند سيطبع، جرّب صفحة فعلية أو معاينة طباعة لأن الهوامش المرئية على الشاشة قد تختلف عن المساحة المتاحة على الورق.",
    "safe-multi-step-file-workflows": "وسجّل اسم المسار والخطوات المستخدمة عندما تكون النتيجة مهمة، لأن قابلية إعادة نفس العملية لاحقًا جزء من الجودة وليست مجرد راحة إضافية.",
    "browser-local-tools-and-privacy": "وعند الشك افتح أدوات المطور أو وثائق الخصوصية للتأكد من حدود المعالجة، فالمصطلح التسويقي وحده لا يكفي لاتخاذ قرار حساس.",
}
...
for _post in V8_BLOG_POSTS:
    ...
    _tail = _AR_REVIEW_TAIL.get(_post["slug"])
    if _tail:
        _title, _paragraph = _post["sections"][-1]
        _post["sections"][-1] = (_title, _paragraph + " " + _tail)
