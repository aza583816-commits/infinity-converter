...
if RATE_LIMIT_STORAGE_URI == "memory://" and os.getenv("RAILWAY_ENVIRONMENT"):
    logger.warning(
        "Rate limits are using memory:// and are therefore process-local. "
        "Configure RATE_LIMIT_STORAGE_URI or REDIS_URL before scaling web workers/replicas or enabling paid AI traffic."
    )
...
def limiter_storage_uri() -> str:
    """Return the configured backend type for diagnostics/tests without exposing secrets."""
