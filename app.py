...
def bad_signature_response(is_arabic):
    return bad_request("نوع الملف الفعلي لا يطابق الامتداد المتوقع أو الملف تالف." if is_arabic else "The file's actual content does not match the expected type, or it is corrupted.")

def enforce_pdf_page_limit(reader, is_arabic):
    if len(reader.pages) > MAX_PDF_PAGES:
        return bad_request(f"عدد صفحات الملف يتجاوز الحد المسموح ({MAX_PDF_PAGES} صفحة)." if is_arabic else f"The file exceeds the maximum allowed pages ({MAX_PDF_PAGES}).")
    return None
...
def apply_ghost_privacy(writer):
    """الميزة الـ VIP: وضع الشبح لمسح بيانات التتبع من الـ PDF"""
    writer.add_metadata({
        "/Author": "",
        "/Creator": "",
        "/Producer": "",
...
def auto_fit_excel_columns(writer, sheet_name="Sheet1", add_autofilter=True):
    """الميزة الـ VIP: تنسيق الجداول بشكل فاخر مع فلاتر"""
...
def ensure_arabic_font():
    ...
    if not os.path.exists(font_path):
        try:
            ...
            pass
    for path in [font_path, "static/fonts/NotoNaskhArabic-Regular.ttf", "static/Cairo-Regular.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", "/usr/share/fonts/truetype/freefont/FreeSans.ttf"]:
...
def get_file_bytes(payload, key="fileBase64"):
    ...
    if not re.match(r'^[A-Za-z0-9+/]+={0,2}$', b64_clean):
        raise ValueError("بيانات الملف غير صالحة أمنياً")
...
def run_libreoffice_convert(src_path, out_dir):
    cmd = ["libreoffice", "--headless", "--nologo", "--nofirststartwizard", "--norestore", "--convert-to", "pdf", src_path, "--outdir", out_dir]
...
def handle_pdf_to_pdf_enhanced(p):
    ...
    except PdfReadError: return bad_request("الملف تالف أو محمي بكلمة سر" if is_arabic else "The file is corrupted or password-protected")
    except Exception: return bad_request("تعذر قراءة الملف" if is_arabic else "Could not read PDF")
...
def handle_merge_word(p):
    ...
    if len(files) < 2: return bad_request("يرجى رفع ملفين Word على الأقل للدمج" if is_arabic else "Please upload at least 2 Word files")
    if len(files) > MAX_MERGE_FILES: return bad_request(f"الحد الأقصى {MAX_MERGE_FILES} ملفات" if is_arabic else f"Maximum {MAX_MERGE_FILES} files")
...
def handle_merge_pdf(p):
    ...
    if len(files) < 2: return bad_request("يرجى رفع ملفين PDF على الأقل للدمج" if is_arabic else "Please upload at least 2 PDFs")
    if len(files) > MAX_MERGE_FILES: return bad_request(f"الحد الأقصى {MAX_MERGE_FILES} ملفات لكل عملية دمج" if is_arabic else f"Maximum {MAX_MERGE_FILES} files per merge")
    ...
    for b64 in files:
        try: raw = base64.b64decode(b64, validate=True)
        except Exception: return bad_request("أحد الملفات غير صالح" if is_arabic else "One of the files is invalid")
        if not validate_signature(raw, "pdf"): return bad_signature_response(is_arabic)
        try: reader = PdfReader(io.BytesIO(raw))
        except PdfReadError: return bad_request("أحد الملفات تالف أو محمي بكلمة سر" if is_arabic else "One of the files is corrupted or password-protected")
        total_pages += len(reader.pages)
        if total_pages > MAX_PDF_PAGES: return bad_request(f"إجمالي الصفحات يتجاوز الحد المسموح ({MAX_PDF_PAGES})" if is_arabic else f"Total pages exceed the limit ({MAX_PDF_PAGES})")
...
def handle_rotate_pdf(p):
    ...
    if angle not in (90, 180, 270): return bad_request("الزاوية يجب أن تكون 90 أو 180 أو 270" if is_arabic else "Angle must be 90, 180, or 270")
...
def handle_protect_pdf(p):
    ...
    if not file_bytes: return bad_request("No file provided")
    if not password or len(password) < 4: return bad_request("يرجى إدخال كلمة سر لا تقل عن 4 أحرف" if is_arabic else "Please provide a password of at least 4 characters")
    if not validate_signature(file_bytes, "pdf"): return bad_signature_response(is_arabic)
    try: reader = PdfReader(io.BytesIO(file_bytes))
    except PdfReadError: return bad_request("الملف تالف" if is_arabic else "The file is corrupted")
    err = enforce_pdf_page_limit(reader, is_arabic)
    if err: return err
...
def handle_unlock_pdf(p):
    ...
    try:
        ...
        if reader.is_encrypted:
            ...
            if not ok: return bad_request("كلمة السر غير صحيحة" if is_arabic else "Incorrect password")
    except PdfReadError: return bad_request("الملف تالف" if is_arabic else "The file is corrupted")
    err = enforce_pdf_page_limit(reader, is_arabic)
    if err: return err
...
def handle_watermark_pdf(p):
    ...
    if not file_bytes: return bad_request("No file provided")
    if not watermark_text: return bad_request("يرجى إدخال نص العلامة المائية" if is_arabic else "Please provide watermark text")
    if not validate_signature(file_bytes, "pdf"): return bad_signature_response(is_arabic)
    try: reader = PdfReader(io.BytesIO(file_bytes))
    except PdfReadError: return bad_request("الملف تالف أو محمي بكلمة سر" if is_arabic else "The file is corrupted or password-protected")
    err = enforce_pdf_page_limit(reader, is_arabic)
...
def handle_excel_to_json(p):
    ...
    except Exception: return bad_request("تعذر قراءة ملف الإكسل" if is_arabic else "Could not read the Excel file")
    
    df = df.map(lambda x: x.strip() if isinstance(x, str) else x)
    df = df.where(pd.notnull(df), None) # الميزة الـ VIP: إبادة القيم الميتة (NaN Purge)
...
def _load_validated_image(p, is_arabic):
    ...
    try: img = open_image_safely(file_bytes)
    except Image.DecompressionBombError: return None, bad_request("أبعاد الصورة كبيرة جداً وغير آمنة للمعالجة" if is_arabic else "Image dimensions are unsafely large")
...
def handle_compress_image(p):
    ...
    except (TypeError, ValueError): quality = 70

    img = img.convert("RGB")
    img = ImageOps.exif_transpose(img)
    
    # الميزة الـ VIP: الفلتر السينمائي
...
def handle_image_to_base64(p):
    ...
    if not re.fullmatch(r"image/[a-zA-Z0-9.+-]+", mime or ""): mime = "image/png"
...
def handle_heic_to_jpg(p):
    ...
    except Image.DecompressionBombError: return bad_request("أبعاد الصورة كبيرة جداً وغير آمنة للمعالجة" if is_arabic else "Image dimensions are unsafely large")
    buf = io.BytesIO()
...
def handle_resize_image(p):
    ...
    except (TypeError, ValueError): return bad_request("قيم الأبعاد غير صحيحة" if is_arabic else "Invalid dimensions")
    if target_w <= 0 and target_h <= 0: return bad_request("يرجى تحديد العرض أو الارتفاع" if is_arabic else "Please specify width or height")
    if target_w > 8000 or target_h > 8000: return bad_request("الأبعاد المطلوبة كبيرة جداً" if is_arabic else "Requested dimensions are too large")
...
def handle_hmac_generator(p):
    ...
    text = p.get("text", "")
    key = p.get("key", "")
    if not key: return bad_request("يرجى إدخال المفتاح السري" if is_arabic else "Please provide a secret key")
...
def handle_text_to_qr(p):
    """الميزة الـ VIP: الباركود الملكي الفخم مع اللوقو"""
    text = p.get("text", "")
    is_arabic = p["is_arabic"]
    if not text.strip(): return bad_request("يرجى إدخال نص أو رابط" if is_arabic else "Please enter text or a link")
    if len(text) > 2000: return bad_request("النص طويل جداً لرمز QR" if is_arabic else "Text is too long for a QR code")
...
def handle_password_generator(p):
    """الميزة الـ VIP: مولد كلمات سر بنكية خالية من الحروف المبهمة"""
    try: length = int(p.get("length", 20))
    except (TypeError, ValueError): length = 20
    ...
    if use_symbols: password_chars.append(secrets.choice(symbol_chars))
    password_chars += [secrets.choice(chars) for _ in range(length - len(password_chars))]
    secrets.SystemRandom().shuffle(password_chars)
    
    pwd = "".join(password_chars)
    formatted_pwd = "-".join([pwd[i:i+4] for i in range(0, len(pwd), 4)])
    return jsonify({"result": formatted_pwd})
...
def convert():
    ...
    try:
        ...
        # الميزة الـ VIP 🚀: تنظيف عميق للذاكرة عشان السيرفر ما يعلق
        gc.collect() 
        return response
    except Exception:
