...
def enforce_pdf_page_limit(page_count, is_arabic):
    if page_count > MAX_PDF_PAGES: return bad_request(f"يتجاوز الحد المسموح ({MAX_PDF_PAGES} صفحة)." if is_arabic else f"Exceeds maximum pages ({MAX_PDF_PAGES}).")
    return None
...
def handle_pdf_to_docx(p):
    ...
    except subprocess.TimeoutExpired: return bad_request("استغرقت المعالجة وقتاً طويلاً جداً. الملف معقد." if is_arabic else "Processing took too long.")
    except Exception: return bad_request("فشل التحويل. قد يكون الملف تالفاً." if is_arabic else "Conversion failed.")
    finally:
...
# ================= أدوات الطلاب، المعلمين والذكاء الاصطناعي =================
def handle_image_to_text(p):
    ...
    try:
        ...
        return bad_request("فشل التعرف على النص. يرجى التأكد من تثبيت الحزم.")
...
def handle_translate_text(p):
    ...
    except Exception: return bad_request("فشل الترجمة، يرجى المحاولة لاحقاً.")
...
REGISTRY = {
    ...
    "base64-tool": handle_base64_tool, "url-encoder": handle_url_encoder, "json-beautifier": handle_json_beautifier, 
    "css-js-minifier": handle_css_js_minifier, "html-entity": handle_html_entity, "hash-generator": handle_hash_generator, 
    "hmac-generator": handle_hmac_generator, "timestamp-converter": handle_timestamp_converter, "clean-text": handle_clean_text, 
    ...
    "unit-converter": handle_unit_converter, "uuid-generator": handle_uuid_generator, "markdown-to-html": handle_markdown_to_html, 
    "text-diff": handle_text_diff, "text-to-audio": handle_text_to_audio, "translate-text": handle_translate_text, 
    "watermark-pdf": handle_watermark_pdf, "remove-pdf-pages": handle_remove_pdf_pages
}
...
def convert():
    ...
    for b64 in files_to_check:
        if b64 and (len(b64) * 3 / 4) > MAX_FILE_BYTES:
            return jsonify({"error": f"حجم الملف أكبر من الحد المسموح ({MAX_FILE_MB}MB)"}), 413
    ...
    try:
        ...
        # تنظيف إجباري للذاكرة لحماية السيرفر
        gc.collect() 
        return response
