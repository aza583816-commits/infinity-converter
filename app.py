...
# ================= المكتبات الخارقة للـ PDF والذكاء الاصطناعي (كلها اختيارية وتتدهور بأمان) =================
try:
    import fitz  # PyMuPDF
except Exception:
    fitz = None
...
# ملاحظة: القيم الافتراضية هنا معايرة لاستضافة Render مدفوعة من الفئة الصغيرة/المتوسطة (Starter/Standard).
# إن كانت خطتك أعلى (RAM/CPU أكبر)، ارفعها عبر متغيرات البيئة بدل تعديل الكود.
MAX_FILE_MB = int(os.environ.get("MAX_FILE_MB", 12))
MAX_FILE_BYTES = MAX_FILE_MB * 1024 * 1024
MAX_MERGE_FILES = int(os.environ.get("MAX_MERGE_FILES", 15))
MAX_PDF_PAGES = int(os.environ.get("MAX_PDF_PAGES", 400))
MAX_OCR_PAGES = int(os.environ.get("MAX_OCR_PAGES", 25))  # OCR مكلف جداً بالمعالجة؛ حد منفصل وأشد
MAX_TEXT_CHARS = int(os.environ.get("MAX_TEXT_CHARS", 3_000_000))
SUBPROCESS_TIMEOUT = int(os.environ.get("SUBPROCESS_TIMEOUT", 90))
...
Image.MAX_IMAGE_PIXELS = int(os.environ.get("MAX_IMAGE_PIXELS", 60_000_000))
...
# OCR والتحويل الصوتي/الترجمة تعتمد على استدعاءات خارجية أو معالجة صور ثقيلة، فتأخذ أشد حد
HEAVY_ACTIONS = {
...
def dynamic_convert_limit():
    ...
    return "8 per minute" if payload.get("action") in HEAVY_ACTIONS else "30 per minute"


@app.after_request
def set_secure_headers(response):
    # رؤوس أساسية آمنة على كل الصفحات، لا تعطل تحميل CSS/خطوط/سكربتات خارجية
    ...
    if request.path == "/convert":
        # سياسة صارمة على مسار الـ API فقط، لأنه لا يعرض HTML ولا يحتاج موارد خارجية
...
def ratelimit_handler(e):
    return jsonify(error="تم تجاوز الحد المسموح. يرجى الانتظار قليلاً."), 429


@app.errorhandler(413)
def too_large_handler(e):
    return jsonify(error="حجم الطلب أكبر من الحد المسموح."), 413
...
# ==================== القائمة الشاملة لجميع الأدوات (للسايتماب وصفحات الـ SEO) ====================
TOOLS_DEF = [
...
def enforce_pdf_page_limit(page_count, is_arabic):
    if page_count > MAX_PDF_PAGES:
        return bad_request(f"يتجاوز عدد الصفحات الحد المسموح ({MAX_PDF_PAGES} صفحة)." if is_arabic
                            else f"Exceeds maximum pages ({MAX_PDF_PAGES}).")
    return None


def apply_ghost_privacy(writer):
    """يمسح بيانات التعريف (المؤلف، البرنامج المستخدم، تواريخ الإنشاء) من ملفات PDF الناتجة لحماية خصوصية المستخدم."""
    try:
...
def run_libreoffice_convert(src_path, out_dir):
    subprocess.run(
        ["libreoffice", "--headless", "--nologo", "--nofirststartwizard", "--norestore", "--convert-to", "pdf", src_path, "--outdir", out_dir],
        check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=SUBPROCESS_TIMEOUT
    )
...
# ==================== دوال بناء المستندات من نص/CSV خام (كانت مفقودة بالنسخة السابقة!) ====================
...
# ==================== طبقة الذكاء الاصطناعي: OCR للـ PDF الممسوح ضوئياً بدون طبقة نص ====================
def ocr_pdf_page_to_text(fitz_page, lang):
    """يرسم صفحة PDF كصورة عالية الدقة (x2) ثم يشغّل Tesseract OCR عليها.
    يُستخدم فقط عندما يفشل الاستخراج العادي (أي أن الصفحة صورة ممسوحة، لا نص حقيقي)."""
...
def is_probably_scanned(text, page_count):
    """يقدّر ما إذا كان المستند صورة ممسوحة (نص شبه فارغ) بدل نص حقيقي قابل للاستخراج."""
    if page_count == 0:
        return False
...
# ================= أدوات الـ PDF الأساسية (مع تعزيز OCR للملفات الممسوحة ضوئياً) =================
...
def handle_pdf_to_docx(p):
    ...
    try:
        ...
        return bad_request("استغرقت المعالجة وقتاً طويلاً جداً. الملف معقد." if is_arabic else "Processing took too long.")
    except Exception:
        return bad_request("فشل التحويل. قد يكون الملف تالفاً." if is_arabic else "Conversion failed.")


def handle_pdf_to_excel(p):
    """استخراج جداول بدقة عالية عبر pdfplumber، مع احتياط PyMuPDF/pypdf، وأخيراً OCR للملفات الممسوحة."""
    file_bytes = get_file_bytes(p)
    is_arabic = p["is_arabic"]
    if not file_bytes:
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        # المرحلة 1: pdfplumber لاكتشاف جداول حقيقية (خطوط/حدود)
        if pdfplumber:
            try:
        ...
        # المرحلة 2: PyMuPDF كاحتياط أسرع لاستخراج النصوص كصفوف
        ...
        # المرحلة 3: pypdf كملاذ أخير بدون مكتبات اختيارية
        if not has_data:
        ...
        # المرحلة 4 (الذكاء الاصطناعي): لو ما زال بدون بيانات، غالباً الملف صورة ممسوحة ضوئياً — نجرب OCR
        if not has_data and fitz and pytesseract and page_count and page_count <= MAX_OCR_PAGES:
...
def handle_pdf_to_csv(p):
    ...
    try:
        ...
        # ملاذ OCR أخير لو الملف ممسوح ضوئياً بالكامل
        if not wrote_any and fitz and pytesseract and page_count and page_count <= MAX_OCR_PAGES:
            lang = 'ara+eng' if is_arabic else 'eng'
            doc = fitz.open(stream=file_bytes, filetype="pdf")
            for page in doc:
...
def handle_pdf_to_text(p):
    ...
    try:
        ...
        # طبقة الذكاء الاصطناعي: لو الملف يبدو ممسوحاً ضوئياً بدون نص حقيقي، نلجأ لـ OCR تلقائياً
        used_ocr = False
        ...
        return bad_request("الملف تالف أو تعذر استخراج النص" if is_arabic else "Could not read the PDF")
...
def handle_merge_pdf(p):
    ...
    is_arabic = p["is_arabic"]
    if len(files) < 2:
        return bad_request("يرجى رفع ملفين PDF على الأقل" if is_arabic else "Please upload at least 2 PDFs")
...
def handle_rotate_pdf(p):
    """أداة: تدوير جميع صفحات PDF بزاوية 90/180/270."""
    is_arabic = p["is_arabic"]
    file_bytes = get_file_bytes(p)
    if not file_bytes:
        return bad_request("No file provided")
...
def handle_compress_pdf(p):
    """أداة: ضغط PDF عبر تقليل محتوى التدفقات (content streams)."""
    is_arabic = p["is_arabic"]
    file_bytes = get_file_bytes(p)
    if not file_bytes:
        return bad_request("No file provided")
...
def handle_protect_pdf(p):
    """أداة: حماية PDF بكلمة سر (تشفير AES-256)."""
    is_arabic = p["is_arabic"]
    file_bytes = get_file_bytes(p)
...
def handle_unlock_pdf(p):
    """أداة: إزالة كلمة سر معروفة من ملف PDF."""
    is_arabic = p["is_arabic"]
    file_bytes = get_file_bytes(p)
    password = p.get("password", "")
    if not file_bytes:
...
def handle_watermark_pdf(p):
    ...
    try:
        ...
        return bad_request("فشل إضافة العلامة المائية." if is_arabic else "Failed to add watermark.")
...
def handle_remove_pdf_pages(p):
    ...
    if not validate_signature(file_bytes, "pdf"):
        return bad_signature_response(is_arabic)
    if not text:
        return bad_request("يرجى كتابة أرقام الصفحات المراد حذفها (مثال: 1, 3, 5-7)" if is_arabic else "Please specify page numbers to remove (e.g. 1, 3, 5-7)")
    try:
        reader = PdfReader(io.BytesIO(file_bytes))
        err = enforce_pdf_page_limit(len(reader.pages), is_arabic)
        if err:
        ...
        if len(writer.pages) == 0:
            return bad_request("لا يمكنك حذف جميع صفحات الملف!" if is_arabic else "You cannot remove all pages!")
        apply_ghost_privacy(writer)
        final_buf = io.BytesIO()
        writer.write(final_buf)
        return file_response(final_buf.getvalue(), "application/pdf", "Edited_Document.pdf")
    except Exception:
        return bad_request("فشل قص الصفحات، يرجى كتابة الأرقام بشكل صحيح." if is_arabic else "Failed to remove pages; check the page numbers.")
...
def handle_word_to_pdf(p):
    ...
    if file_bytes:
        ...
        try:
            ...
            return bad_request("استغرقت المعالجة وقتاً طويلاً." if is_arabic else "Processing took too long.")
        except Exception:
            return bad_request("فشل التحويل. قد يكون السيرفر تحت ضغط." if is_arabic else "Conversion failed due to server load.")
...
def handle_text_to_pdf(p):
    if not p.get("text", "").strip():
        return bad_request("يرجى إدخال نص" if p["is_arabic"] else "Please enter text")
...
def handle_merge_word(p):
    ...
    if len(files) < 2:
        return bad_request("يرجى رفع ملفين Word على الأقل" if is_arabic else "Please upload at least 2 Word files")
    if len(files) > MAX_MERGE_FILES:
        return bad_request(f"الحد الأقصى {MAX_MERGE_FILES} ملفات" if is_arabic else f"Maximum {MAX_MERGE_FILES} files")
    merged = Document()
    first = True
...
def handle_image_to_base64(p):
    ...
    if not re.fullmatch(r"image/[a-zA-Z0-9.+-]+", mime):
        mime = "image/png"
...
def handle_heic_to_jpg(p):
    if pillow_heif is None:
        return bad_request("pillow-heif غير مثبّت")
    file_bytes = get_file_bytes(p)
    is_arabic = p["is_arabic"]
    if not file_bytes:
    ...
    try:
        ...
        return bad_request("أبعاد الصورة كبيرة جداً" if is_arabic else "Image is too large")
    buf = io.BytesIO()
...
def handle_resize_image(p):
    """أداة: تغيير أبعاد الصورة مع خيار الحفاظ على النسبة."""
    is_arabic = p["is_arabic"]
...
def handle_strip_exif(p):
    """أداة: إزالة بيانات EXIF (الموقع الجغرافي وغيره) من الصورة لحماية الخصوصية."""
    is_arabic = p["is_arabic"]
    img, err = _load_validated_image(p, is_arabic)
    if err:
        return err
...
def handle_image_to_text(p):
    if pytesseract is None:
        return bad_request("مكتبة OCR غير مثبتة بالسيرفر" if p["is_arabic"] else "OCR library is not installed on the server")
    img, err = _load_validated_image(p, p["is_arabic"])
    if err:
        return err
    try:
        ...
        if not text.strip():
            return jsonify({"result": "لم يتم العثور على أي نص واضح في الصورة." if p["is_arabic"] else "No readable text was found in the image."})
        return jsonify({"result": text.strip()})
    except Exception:
        return bad_request("فشل التعرف على النص. يرجى التأكد من تثبيت Tesseract على السيرفر." if p["is_arabic"] else "OCR failed. Make sure Tesseract is installed on the server.")


def handle_text_to_audio(p):
    if gTTS is None:
        return bad_request("مكتبة الصوت غير مثبتة." if p["is_arabic"] else "Audio library is not installed.")
    text = p.get("text", "").strip()
    if not text:
        return bad_request("يرجى إدخال النص." if p["is_arabic"] else "Please enter text.")
    if len(text) > 5000:
        return bad_request("النص طويل جداً (الحد الأقصى 5000 حرف)." if p["is_arabic"] else "Text is too long (max 5000 characters).")
    try:
        ...
        return bad_request("فشل توليد الصوت. تأكد من الاتصال بالإنترنت من السيرفر." if p["is_arabic"] else "Failed to generate audio.")


def handle_translate_text(p):
    if GoogleTranslator is None:
        return bad_request("مكتبة الترجمة غير مثبتة." if p["is_arabic"] else "Translation library is not installed.")
    text = p.get("text", "").strip()
    if not text:
        return bad_request("يرجى إدخال النص." if p["is_arabic"] else "Please enter text.")
    if len(text) > 4500:
        text = text[:4500]  # خدمة الترجمة غير الرسمية تفشل عادةً على نصوص أطول من ~5000 حرف
    try:
        target_lang = 'en' if p["is_arabic"] else 'ar'
        translated = GoogleTranslator(source='auto', target=target_lang).translate(text)
        return jsonify({"result": translated})
    except Exception:
        return bad_request("فشلت الترجمة، يرجى المحاولة لاحقاً أو بنص أقصر." if p["is_arabic"] else "Translation failed; try again or use shorter text.")
...
def handle_json_beautifier(p):
    try:
        return jsonify({"result": json.dumps(json.loads(p.get("text", "")), ensure_ascii=False, indent=4, sort_keys=True)})
    except Exception:
...
def handle_hmac_generator(p):
    ...
    if not key:
        return bad_request("يرجى إدخال المفتاح السري" if p["is_arabic"] else "Please provide a secret key")
...
def handle_timestamp_converter(p):
    try:
        return jsonify({"result": datetime.fromtimestamp(int(p.get("text", "").strip()), tz=timezone.utc).strftime("%a, %d %b %Y %H:%M:%S GMT")})
    except Exception:
...
def handle_text_to_qr(p):
    ...
    text = p.get("text", "")
    if not text.strip():
        return bad_request("يرجى إدخال نص أو رابط" if is_arabic else "Please enter text or a link")
    if len(text) > 2000:
        return bad_request("النص طويل جداً" if is_arabic else "Text is too long")
...
def handle_password_generator(p):
    try:
        length = max(8, min(128, int(p.get("length", 20))))
    except Exception:
        length = 20
    ...
    if use_symbols:
        pw_chars.append(secrets.choice("!@#$%^&*_+="))
...
def handle_password_strength(p):
    text = p.get("text", "")
    score = sum([
        len(text) >= 8, len(text) >= 12,
        bool(re.search(r"[A-Z]", text)), bool(re.search(r"[a-z]", text)),
        bool(re.search(r"[0-9]", text)), bool(re.search(r"[^A-Za-z0-9]", text)),
    ])
    labels = (["ضعيفة جداً ⚠️", "ضعيفة ⚠️", "متوسطة 🟡", "جيدة 🙂", "قوية 🔒", "قوية جداً 🔒🔒", "ممتازة 🛡️"]
              if p["is_arabic"] else
              ["Very Weak ⚠️", "Weak ⚠️", "Fair 🟡", "Good 🙂", "Strong 🔒", "Very Strong 🔒🔒", "Excellent 🛡️"])
...
REGISTRY = {
    ...
    "text-to-audio": handle_text_to_audio, "translate-text": handle_translate_text,
}
...
def convert():
    ...
    try:
        ...
        gc.collect()
        return jsonify({"error": "حدث خطأ أثناء المعالجة. تأكد من صحة الملف وحاول مجدداً." if is_arabic else "An error occurred while processing. Please check the file and try again."}), 500
