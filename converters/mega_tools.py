"""Legacy high-value converter implementations retained during the 7.1 migration.

Public routing no longer dispatches through this module directly: the 7.1 operation
registry wraps these proven local implementations while they are split by domain in
future maintenance releases. Keeping this compatibility layer avoids breaking working
conversions during the architectural migration.
"""
...
def _ocr_text_file(source: Path, lang: str = "ar+eng") -> str:
    ...
    with Image.open(source) as img:
        return pytesseract.image_to_string(ImageOps.exif_transpose(img), lang=_ocr_lang(lang), timeout=settings.ocr_timeout_seconds)
...
def ocr_pdf_to_markdown(source: Path, output: Path, lang: str = "ar+eng"):
    ...
    try:
        if pdf.page_count > settings.max_ocr_pages:
            raise ValueError(f"عدد صفحات OCR يتجاوز الحد المسموح ({settings.max_ocr_pages}).")
        for i,p in enumerate(pdf,1):
            pix=p.get_pixmap(dpi=160,alpha=False)
            with Image.open(io.BytesIO(pix.tobytes("png"))) as img:
                text=pytesseract.image_to_string(img, lang=_ocr_lang(lang), timeout=settings.ocr_timeout_seconds)
...
def _bounded_decompress(stream, output: Path):
    total = 0
    with stream as src, output.open("wb") as dst:
        while True:
...
def bzip2_decompress(source: Path, output: Path):
    try:
        _bounded_decompress(bz2.open(source, "rb"), output)
    except (OSError, EOFError) as exc:
        output.unlink(missing_ok=True)
        raise ValueError("ملف BZIP2 غير صالح.") from exc
    ...
    try:
        with tarfile.open(source, mode) as tf:
            ...
            if len(members) > settings.max_archive_entries:
                raise ValueError("الأرشيف يحتوي على عدد عناصر غير طبيعي.")
            total = 0
            for member in members:
                parts = Path(member.name).parts
                if Path(member.name).is_absolute() or ".." in parts or not (member.isfile() or member.isdir()):
                    raise ValueError("الأرشيف يحتوي على مسار أو نوع عنصر غير آمن.")
        ...
        raise ValueError("الأرشيف غير صالح.") from exc
    return outputs

def tar_gzip_extract(source: Path, output_dir: Path):
    return _safe_tar_extract(source, output_dir, "r:gz")
...
def tar_bzip2_extract(source: Path, output_dir: Path):
    return _safe_tar_extract(source, output_dir, "r:bz2")
