...
def _bounded_decompress(source: Path, output: Path, decoder_factory, invalid_message: str):
    limit = settings.max_archive_uncompressed_bytes
    total = 0
    try:
        decoder = decoder_factory(source.read_bytes())
...
def bzip2_decompress(source: Path, output: Path):
    _bounded_decompress(source, output, lambda raw: bz2.BZ2File(io.BytesIO(raw), "rb"), "ملف BZIP2 غير صالح.")
...
def xz_decompress(source: Path, output: Path):
    _bounded_decompress(source, output, lambda raw: lzma.LZMAFile(io.BytesIO(raw), "rb"), "ملف XZ غير صالح.")
...
def _safe_tar_extract(source: Path, output_dir: Path, mode: str):
    ...
    with tarfile.open(source, mode) as tf:
        ...
        for m in members:
            name = m.name.replace("\\", "/")
            if not (m.isfile() or m.isdir()) or name.startswith("/") or name.startswith("../") or "/../" in name:
                raise ValueError("الأرشيف يحتوي على مسار أو ملف غير آمن.")
            total += max(0, m.size)
            if m.size > settings.max_archive_uncompressed_bytes or total > settings.max_archive_uncompressed_bytes:
        ...
        for m in members:
            ...
            with src, target.open("wb") as dst:
                ...
                while chunk := src.read(1024 * 1024):
                    copied += len(chunk)
                    if copied > settings.max_archive_uncompressed_bytes:
                        raise ValueError("حجم الملف المستخرج يتجاوز الحد الآمن.")
                    dst.write(chunk)
            out.append(target)
    return out
