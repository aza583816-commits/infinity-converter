  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        ...
        if (document.body.dataset.billingEnabled === "1") {
    ...
    saveRecent($("#tool-id").value, document.title.split(" | ")[0], $("#tool-path")?.value); renderRecent(); status.textContent = i18n("js.completed");
    result.textContent = i18n("js.result_label", { filename, size: Math.ceil(blob.size / 1024) });
    const metrics = $("#result-metrics");
    const formatBytes = (bytes) => bytes >= 1024 * 1024 ? `${(bytes / 1024 / 1024).toFixed(bytes >= 10 * 1024 * 1024 ? 1 : 2)} MB` : `${Math.max(1, Math.ceil(bytes / 1024))} KB`;
    ...
    if (engineNode && engine) { engineNode.textContent = `${document.documentElement.lang === "ar" ? "المحرك" : "Engine"}: ${engine}`; engineNode.hidden = false; }
...
$("#reset-tool")?.addEventListener("click", () => { selectedFiles = []; renderFiles(); if (intelligence) intelligence.hidden = true; if (resultPanel) resultPanel.hidden = true; $("#status").textContent = i18n("js.ready"); $("#result").textContent = ""; const metrics = $("#result-metrics"); if (metrics) metrics.hidden = true; const engineNode = $("#result-engine"); if (engineNode) engineNode.hidden = true; document.querySelectorAll(".batch-summary").forEach((el) => el.remove()); });
...
(() => {
  ...
  document.querySelectorAll('[data-ai-prefill]').forEach((button) => {
    button.addEventListener('click', () => {
      const tool = button.dataset.aiPrefill || 'This tool';
      const box = document.querySelector('#ai-prompt');
      if (!box) {
        window.location.href = `/?ai=${encodeURIComponent(tool)}#infinity-ai`;
        return;
      }
      box.value = `${tool} — suggest the best workflow, useful presets, and the next step for this file.`;
      document.querySelector('#infinity-ai')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      setTimeout(() => box.focus(), 450);
    });
  });
  const queryAI = new URLSearchParams(window.location.search).get('ai');
  if (queryAI) {
    prompt.value = `${queryAI} — suggest the best workflow, useful presets, and the next step for this file.`;
  }
  const refreshAIStatus = async () => {
    try {
      ...
      if (data.ready) {
        status.textContent = document.documentElement.lang === 'ar' ? 'متصل وجاهز' : 'Connected & ready';
        status.dataset.state = 'ready';
      } else {
        status.textContent = data.message || (document.documentElement.lang === 'ar' ? 'غير متاح حاليًا' : 'Unavailable');
        status.dataset.state = 'error';
      }
    } catch (_) {
      status.textContent = document.documentElement.lang === 'ar' ? 'تعذر فحص الاتصال' : 'Connection check failed';
      status.dataset.state = 'error';
    }
  };
  refreshAIStatus();
  form.addEventListener('submit', async (event) => {
    ...
    if (!value) return;
    status.textContent = document.documentElement.lang === 'ar' ? 'يفكر…' : 'Thinking…';
    answer.hidden = false;
    answer.textContent = document.documentElement.lang === 'ar' ? 'جاري الاتصال بـ Infinity AI…' : 'Connecting to Infinity AI…';
    const button = form.querySelector('button[type="submit"]');
    if (button) button.disabled = true;
    try {
      const response = await fetch('/api/v2/ai/ask', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: value })
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'AI request failed.');
      answer.textContent = data.answer || '';
      status.textContent = document.documentElement.lang === 'ar' ? 'تم' : 'Done';
    } catch (error) {
      answer.textContent = error.message || String(error);
      status.textContent = document.documentElement.lang === 'ar' ? 'تعذر الاتصال' : 'Unavailable';
    } finally {
...
// Infinity AI global dock — provider-neutral public experience.
(() => {
  ...
  const prompt = document.getElementById('global-ai-prompt');
  const answer = document.getElementById('global-ai-answer');
  const status = document.getElementById('global-ai-status');
  const state = document.getElementById('ai-dock-state');
  const ar = document.documentElement.lang === 'ar';

  function setOpen(open) {
    panel.hidden = !open;
    toggle.setAttribute('aria-expanded', open ? 'true' : 'false');
    if (open) setTimeout(() => prompt.focus(), 30);
  }
  ...
  async function checkAI() {
    try {
      const response = await fetch('/api/v2/ai/status', { headers: { 'Accept': 'application/json' } });
      const data = await response.json();
      const ready = Boolean(response.ok && data.ready);
      state.dataset.state = ready ? 'ready' : 'error';
      status.textContent = ready ? (ar ? 'Infinity AI جاهز' : 'Infinity AI is ready') : (data.message || (ar ? 'Infinity AI غير متاح مؤقتًا' : 'Infinity AI is temporarily unavailable'));
    } catch (_) {
      state.dataset.state = 'error';
      status.textContent = ar ? 'تعذر التحقق من Infinity AI الآن' : 'Could not verify Infinity AI right now';
    }
  }
  checkAI();
  ...
  form.addEventListener('submit', async (event) => {
    ...
    answer.hidden = false;
    answer.textContent = ar ? 'Infinity AI يفكر…' : 'Infinity AI is thinking…';
    try {
      const response = await fetch('/api/v2/ai/ask', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: value }) });
      const data = await response.json();
      answer.textContent = response.ok ? (data.answer || '') : (data.error || (ar ? 'تعذر إكمال الطلب.' : 'Could not complete the request.'));
    } catch (_) {
      answer.textContent = ar ? 'تعذر الاتصال بـ Infinity AI الآن.' : 'Could not reach Infinity AI right now.';
    } finally {
...
/* 6.1.2 living backdrop: subtle pointer response, never required for use. */
(() => {
