...
if (browserWorkspace) {
  ...
  async function runBrowserTool() {
    ...
    if (toolId === 'lesson-timing-planner') { const minutes = requirePositive(number('minutes'), 'Minutes'); if (!Number.isSafeInteger(minutes)) throw new Error('Enter a whole number of minutes.'); const opening = Math.round(minutes * .1); const instruction = Math.round(minutes * .65); const practice = Math.round(minutes * .15); const review = minutes - opening - instruction - practice; return `Opening: ${opening} min\nInstruction: ${instruction} min\nPractice: ${practice} min\nReview: ${review} min`; }
    ...
    if (toolId === 'break-even-calculator') { const fixedCost = requirePositive(number('fixed'), 'Fixed costs'); const price = requirePositive(number('price'), 'Unit price'); const variable = number('variable'); if (variable < 0 || price <= variable) throw new Error('Unit price must exceed nonnegative variable unit cost.'); return `Break-even units: ${Math.ceil(fixedCost / (price - variable))}`; }
