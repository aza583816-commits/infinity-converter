...
if (form) form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    window.InfinityRuntimeContext.lastResult = { tool_id: $("#tool-id")?.value || "", input_bytes: serverInputBytes, output_bytes: serverOutputBytes || blob.size, duration_ms: serverDurationMs || Math.round(performance.now() - startedAt), engine };
    ...
    if (!result.querySelector(".upgrade-link")) result.textContent = error.message;
    window.InfinityRuntimeContext = window.InfinityRuntimeContext || {};
    window.InfinityRuntimeContext.lastResult = null;
    window.InfinityRuntimeContext.lastError = { tool_id: $("#tool-id")?.value || "", message: String(error?.message || error).slice(0, 900) };
  }
  finally { if (progress) progress.hidden = true; if (submitButton) submitButton.disabled = false; }
});
$("#reset-tool")?.addEventListener("click", () => { window.InfinityRuntimeContext = window.InfinityRuntimeContext || {}; window.InfinityRuntimeContext.lastResult = null; window.InfinityRuntimeContext.lastError = null; selectedFiles = []; renderFiles(); if (intelligence) intelligence.hidden = true; if (resultPanel) resultPanel.hidden = true; $("#status").textContent = i18n("js.ready"); $("#result").textContent = ""; const metrics = $("#result-metrics"); if (metrics) metrics.hidden = true; const engineNode = $("#result-engine"); if (engineNode) engineNode.hidden = true; document.querySelectorAll(".batch-summary").forEach((el) => el.remove()); });
