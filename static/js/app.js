...
function renderRecent() {
  ...
  list.innerHTML = recent.map((item) => `<a href="/tools/${item.id}">${item.name} <span aria-hidden="true">←</span></a>`).join("");
}
...
function renderFavorites() {
  ...
  list.innerHTML = favorites.map((item) => `<a href="/tools/${item.id}">${item.name} <span aria-hidden="true">←</span></a>`).join("");
}
...
(() => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    answer.hidden = false;
    answer.textContent = document.documentElement.lang === 'ar' ? 'جاري الاتصال بـ Gemini…' : 'Connecting to Gemini…';
...
/* Global Infinity AI launcher: available across the entire site. */
(() => {
  const launcher = document.querySelector('#global-ai-launcher');
  const panel = document.querySelector('#global-ai-panel');
  const close = document.querySelector('#global-ai-close');
  const prompt = document.querySelector('#global-ai-prompt');
  const submit = document.querySelector('#global-ai-submit');
  const status = document.querySelector('#global-ai-status');
  const answer = document.querySelector('#global-ai-answer');
  if (!launcher || !panel || !prompt || !submit) return;
  const ar = document.documentElement.lang === 'ar';
  const setOpen = (open) => { panel.hidden = !open; launcher.setAttribute('aria-expanded', String(open)); if (open) setTimeout(() => prompt.focus(), 50); };
  ...
  const check = async () => {
    try {
      const response = await fetch('/api/v2/ai/status', { headers: { Accept: 'application/json' } });
      const data = await response.json();
      status.textContent = data.ready ? (ar ? 'Gemini متصل وجاهز' : 'Gemini connected & ready') : (data.message || (ar ? 'Gemini غير متاح' : 'Gemini unavailable'));
      status.dataset.state = data.ready ? 'ready' : 'error';
    } catch (_) { status.textContent = ar ? 'تعذر فحص Gemini' : 'Gemini status check failed'; status.dataset.state = 'error'; }
  };
  check();
  submit.addEventListener('click', async () => {
    const value = prompt.value.trim(); if (!value) { prompt.focus(); return; }
    submit.disabled = true; status.textContent = ar ? 'Gemini يفكر…' : 'Gemini is thinking…'; answer.hidden = false; answer.textContent = ar ? 'جاري الاتصال…' : 'Connecting…';
    try {
      const response = await fetch('/api/v2/ai/ask', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify({ prompt: value }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || (ar ? 'تعذر الحصول على رد.' : 'AI request failed.'));
      answer.textContent = data.answer || ''; status.textContent = ar ? 'تم الرد' : 'Answer ready'; status.dataset.state = 'ready';
    } catch (error) { answer.textContent = error.message || String(error); status.textContent = ar ? 'تعذر الاتصال' : 'Unavailable'; status.dataset.state = 'error'; }
    finally { submit.disabled = false; }
  });
})();

/* Knowledge center enhancements: keyboard search + reading progress. */
(() => {
  ...
  const update = () => {
    ...
    const progress = Math.min(100, Math.max(0, ((window.scrollY - (window.scrollY + rect.top - 80)) / total) * 100));
