...
def _call_gemini(prompt: str, system: str = "") -> str:
    ...
    try:
        ...
        if exc.code in (400, 401, 403, 404, 429):
            messages = {
                400: "Gemini rejected the request. Check the model/API configuration.",
                401: "Gemini authentication failed. Check the Railway API key.",
                403: "Gemini access was denied. Check the API key permissions and billing/quota settings.",
                404: f"Gemini model '{model}' was not found or is unavailable to this key.",
                429: "Gemini rate limit or quota was reached. Try again shortly.",
            }
            raise RuntimeError(messages[exc.code]) from exc
        raise RuntimeError(f"Gemini is temporarily unavailable (HTTP {exc.code}).") from exc
...
def ask():
    ...
    catalog = "\n".join(
        f"- {tool.id}: {tool.name_en} | {tool.name_ar} | /tools/{_meta_for(tool)['slug']}"
