...
def blog_page():
    ...
    if query:
        ...
        posts = [p for p in posts if needle in " ".join([p["title_ar"], p["title_en"], p["description_ar"], p["description_en"]]).casefold()]
    if category:
        posts = [p for p in posts if p["category_en"].casefold() == category.casefold() or p["category_ar"] == category]
    categories = sorted({(p["category_en"], p["category_ar"]) for p in BLOG_POSTS})
    return render_template("blog.html", posts=posts, categories=categories, query=query, category=category)
...
def robots_txt():
    """Serve robots.txt at the crawler-required origin root (not /static)."""
