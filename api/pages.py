...
def _quality_tool_paths() -> set[str]:
    """Return a deliberately small, high-value tool surface for search engines.

    All registered tools remain usable from /tools. The sitemap focuses on
    popular tools in content-rich categories plus tools directly supported by
    manually curated Knowledge Center guides. This avoids presenting hundreds
    of near-identical utility pages as the site's primary publisher content.
    """
    editorial_categories = {"pdf", "images", "office", "ocr", "archive"}
    paths = set()
    for item in popular_tools(limit=30):
        tool = get_tool(item["id"])
        if tool and tool.category in editorial_categories:
            paths.add(tool_url(tool))
    paths.update(
        post["tool"]
        for post in BLOG_POSTS
        if isinstance(post.get("tool"), str) and post["tool"].startswith("/tools/")
    )
    return paths
...
def sitemap():
    ...
    for url in urls:
        ...
        body += f"<url><loc>{base}</loc><lastmod>2026-09-12</lastmod>"
